...
let messageToArray = message.trim().split(" ")
messageToArray = messageToArray.map((item) => item.toLowerCase())
const wordCount = {}

for (let letter of messageToArray) {
  wordCount[letter] ??= 0
  wordCount[letter]++
}

let result = ""
Object.entries(wordCount).forEach((word) => {
